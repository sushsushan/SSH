...
SHOW VARIABLES WHERE Variable_name IN ('version', 'version_comment', 'innodb_version', 'max_connections', 'max_user_connections', 'query_cache_size', 'query_cache_limit', 'query_cache_type', 'log_bin', 'character_set_server', 'collation_server');" 2>/dev/null
...
# 🔹 Total Number of Views, Triggers, Indexes
...
# 🔹 Database Collation & Character Set
echo -e "\n📌 Database Collation & Character Set:"
mysql -u "$DB_USER" -p"$DB_PASS" -D "$DB_NAME" -e "
SELECT schema_name AS 'Database', default_character_set_name AS 'Character Set', default_collation_name AS 'Collation'
FROM information_schema.schemata
WHERE schema_name='$DB_NAME';" 2>/dev/null
...
echo -e "\n📌 Table Row Format (Fixed, Dynamic, Compressed):"
mysql -u "$DB_USER" -p"$DB_PASS" -D "$DB_NAME" -e "
